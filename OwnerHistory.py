...
def main(root,Room,user,id):
    ...
    History["text"] = "Show Booking History so far"
...
def History_command(root,id,lbl):
    ...
    if available != 0 :
        Records = DB.runQuery("select b_id,b_date,Amount,S.name from Booking B, Student S where owner_id = "+str(id)+" AND B.student_id=S.s_id")
        global treev2
        treev2 = ttk.Treeview(root, selectmode = 'browse', columns=(1, 2, 3, 4, 5), show='headings', height=40, style="Treeview")
        ...
        treev2.column("2", width = 50, anchor ='c')
        treev2.column("3", width = 70, anchor ='c')
        treev2.column("4", width = 85, anchor ='c')
        treev2.column("5", width = 140, anchor ='c')
        treev2.heading("1", text="Sr No.")
        treev2.heading("2", text="Booking-ID")
        treev2.heading("3", text ="Date")
        treev2.heading("4", text ="Amount")
        treev2.heading("5", text="Tenant Name")
        i = 1
        for R in Records :
            treev2.insert("", 'end', text =str(R[0]), values =(i,R[0],R[1],R[2],R[3]));    i=i+1
